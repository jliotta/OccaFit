...
function authorize(options) {
  ...
  // console.log('WHAT ARE MY DEFAULTS:', defaults)
  ...
  return function(data, accept){
    ...
    // console.log('WTFFF????', 0 ||  (data._query && data._query.session_id) || 'WHAT');
    // console.log('DATA SESSIONID ------------>', data.cookie[auth.key]);
    // console.log('DATA SESSIONID ACTUAL ======', data.sessionID);
    ...
    // console.log('DATA SESSION ID:', data);
    // console.log('AUTH.STORE.GET', auth.store.get);
    auth.store.get(data.sessionID, function(err, session){
      // console.log('session inside passport.socketio:', session);
      if(err)
